...
s_acm2 %>% tbl_summary(include = c(grp2, ARES_rec, HEBE_rec3,SRES_rec),
                    by = grp2,
                    percent = "col", 
...
s_acm2 %>% tbl_summary(include = c(grp2, age_ed,age_sd,SEX,HAND, MNA),
                    by = grp2,
                    percent = "col", 
                    statistic = list(all_categorical() ~ "{p}"),
                    missing = "no",) %>%   
...
```{r}
modele <- s_acm2 %>% select(poids_sor, ID, SEX, MNA, HAND, age_ed, age_sd, MES_rec,hebe_capa2, grp2)
...
modele %>% tbl_summary(include = c(SEX, MNA, HAND, age_ed,age_sd, MES_rec, hebe_capa2),
                    by = grp2,
                    missing = "no",
...
m3 <- multinom(grp2 ~ SEX + age_ed + age_sd  + MES_rec +MNA + HAND + hebe_capa2, data = modele)
