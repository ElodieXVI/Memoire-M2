...
enf <- read_sas("data/enf.sas7bdat")
sor <- read_sas("data/sor.sas7bdat")
obs <- read_sas("data/obs.sas7bdat")
...
enf$ppla <- case_when(enf$d_anpp == enf$d_ane ~ "1",
                         T ~"0")

sor$ppla <- case_when(sor$d_anpp == sor$d_ane ~ "1",
                      T ~"0")
...
recode_age <- function(var) {
  ...
  var_rec <- var_rec %>%
    fct_recode("0 à 3 ans" = "[0,4)", "4 à 6 ans" = "[4,7)", "7 à 12 ans" = "[7,13)", "13 à 14 ans" = "[13,15)", "15 à 17 ans" = "[15,18)", "18 ans à plus" = "[18,27]")
  print(table(var_rec, useNA = "ifany"))
  return(var_rec)
}
...
act$hebe_capa <- case_when(act$collect == 1 & act$autonomautre == 0 & act$assfam == 0 & act$domicile == 0 ~ "collectif",
                           ...
                           T ~ "tout")
act$hebe_capa2 <- act$hebe_capa %>%
  fct_recode(
    "un" = "assfam",
    "deux" = "assfam et collectif",
    "trois" = "assfam, domicile et collectif",
    "un" = "autonom",
    "deux" = "autonom et assfam",
    "un" = "collectif",
    "deux" = "collectif et autonome",
    "trois" = "collectif, autonom et assfam",
    "un" = "domicile",
    "deux" = "assfam et domicile",
    "trois" = "autonom, assfam et domicile")

red <- act %>% select("ID", "hebe_capa", "hebe_capa2")

enf <- merge(enf, red, by = "ID")
sor <- merge(sor, red, by = "ID")
# Pondération ####
