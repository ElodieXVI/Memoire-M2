...
require(nnet)
m1 <- multinom(grp ~ age_ed + SEX + MNA + HAND + MES_rec2, data = e_acm)
...
```{r}
modele <- enf %>% select(SEX, MNA, HAND, age_ed, age_pp, MES_rec2, HEBE_rec2, ARES_rec)
...
m3 <- multinom(HEBE_rec2 ~ age_ed + SEX + MNA + HAND + ARES_rec + ARES_rec*SEX, data = modele)
