...
e_acm <- enf %>% subset(CATEG_rec == "MECS") %>% 
  select("SEX","HEBE_rec3", "HAND", "MNA", "MES_rec", 
         "AMES_rec",  "age_ed","ARES_rec", "SEQ", "poids_enf")%>% 
...
```{r}
e_acm$grp <- res.HCPC$data.clust$clust
e_acm %>% tbl_summary(include = c(MES_rec2),
                    by = grp,
                    percent = "col", 
...
```{r}
modele <- e_acm %>% select(poids_enf, SEX, MNA, HAND, age_ed, MES_rec2, HEBE_rec1,HEBE_rec2,HEBE_rec3, ARES_rec, grp)
