...
différents types d'établissement (voir Table 3.1.1). Bien que les MECS
...
l'étude de A. Abassi[@abassi61000Enfants2020] (Voir Table 3.1.2).
...
autres types d'établissement (Voir Table 3.1.3). L'hébergement à domicile est aussi
...
concerne 12% des enfants présents au 15 décembre 2017 (Voir Table 3.1.4). Ces résultats
...
plus présents en internat collectif (Voir Table 3.1.6).
...
(40.3%) (voir Table 3.2.1). Cette situation correspond en partie au cas
...
considération dans l'orientation (voir Table 3.2.2).
...
e_mecsw %>% tbl_svysummary(include = c(ARES_rec, HEBE_rec3),
  ...
  fontsize(size = 9, part = "footer") %>% 
  set_caption(caption = "Le type d’hébergement en MECS en fonction du type d’hébergement avant l’entrée en MECS pour les enfants présents en 2017 (% en ligne pondérés)") %>% 
  color(color = "black", part = "all") %>% 
  width(width = 1) %>% 
  height_all(height = 0.5) %>%
  align(part = "all", align = "center")%>%
...
le même type d'hébergement dans lequel ils étaient auparavant. Une certaine
...
petit à petit vers les types d'hébergements les plus autonomes (Voir Figure 3.2.1).
...
### Une cohérence dans le placement entre hébergement en MECS et à la sortie...
...
enfants sortis en 2017 (Voir Table 3.3.1).

Lorsque l'on regarde dans quels types d'hébergement les enfants sortis
de MECS vont, on observe qu'une forme de cohérence dans le parcours est recherchée.
En effet, il existe tout d'abord une continuité dans le placement
...
à celles des deux hébergements de MECS(Voir Table 3.3.2).
...
s_mecsw %>% tbl_svysummary(include = c(HEBE_rec3),
  ...
  fontsize(size = 9, part = "footer") %>% 
  set_caption(caption = "Le type d’hébergement juste à la sortie de MECS en fonction du type d’hébergement en MECS pour les enfants sortis en 2017 (% en ligne pondérés)") %>% 
  color(color = "black", part = "all") %>% 
  width(width = 1) %>% 
  height_all(height = 0.5) %>%
  align(part = "all", align = "center")%>%
...
répartition entre hébergements, nous soupçonnons que l'âge à la sortie de MECS agissent de même sur le type d'hébergement après le passage en MECS. On remarque dès lors l'apparition croissante avec
...
accompagnés (voir Figure 3.3.1)\footnote{À noter que la question du
...
type d'hébergement avant l'entrée en MECS et l'orientation une fois en
MECS. Du fait d'une absence de données sur le parcours de l'enfant,
alliée à la méthode employée pour l'observer, ces effets sont tout de
même susceptibles d'être existants, mais de nous avoir échappé. Dès
lors, notre regard s'est porté sur le lien entre hébergement en MECS et hébergement
à la sortie de MECS. Ici, nous avons constaté une volonté de cohérence
dans le placement qui était relative à l'âge de l'enfant sorti.
Concrètement, on observe plus de situations où le type d'hébergement en
