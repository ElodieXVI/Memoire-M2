...
chisq.test(svytable(~HEBE_rec3 +ARES_rec, e_mecsw)[,-4])
chisq.residuals(svytable(~HEBE_rec3 +ARES_rec, e_mecsw)[,-4])
```
```{r}
...
e_mecsw %>% tbl_svysummary(include = c(MES_na, HEBE_rec3),
                    by = MES_na,
                    missing = "no",
                    statistic = list(all_categorical() ~ "{p}")) %>%   
  modify_header(label = "") %>% 
