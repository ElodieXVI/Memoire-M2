...
e_mecs$ARES_rec <- e_mecs$ARES_rec %>%
  fct_relevel(
    "Collectif", "Assistant familial", "Famille", "Logt accompagné",
    "Logt hors", "Autres"
  )
...
```{r}
g1 <- e_mecs %>% subset(age_ed == "0_3") %>% 
  drop_na() %>% 
  ggplot() +
  aes(x = HEBE_rec3, fill = ARES_rec, label = f(after_stat(prop))) +
...
g6 <-e_mecs %>% subset(age_ed == "18_+") %>% 
  ...
  scale_fill_manual(values = met.brewer("Hokusai1")) +
  theme_bw() +
  ggtitle("Âge 18 ans à plus")+ theme(text = element_text(family = "Times"))
